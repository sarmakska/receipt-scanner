...
- CSV export. A new `/api/export/csv` endpoint and Export CSV / Export items CSV buttons render scanned receipts as RFC 4180 CSV in two layouts: a per-receipt summary, or one row per purchased line that preserves the full line-item detail OFX cannot carry. Output uses CRLF endings, RFC 4180 quoting, and a UTF-8 BOM so Excel reads accented vendor names correctly. Covered by `lib/csv.test.ts`.
